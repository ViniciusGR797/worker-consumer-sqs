...
class SQSService:
    ...
    def get_queue_url(self, queue_name: str, trace_id: str) -> str | None:
        try:
            response = self.sqs.get_queue_url(QueueName=queue_name)
            queue_url = response["QueueUrl"]
            log_message(
                trace_id, "sqs_get_url", "success", {
                    "queue_name": queue_name, "queue_url": queue_url})
            return queue_url
        except Exception as e:
            log_message(trace_id, "sqs_get_url", "error", {
                "error": str(e), "queue_name": queue_name})
